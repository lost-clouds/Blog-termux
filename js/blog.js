 ...
 */ async function _fetchArticles() {
    ...
    try {
        ...
        const firstMd = _articles.find(function (a) {
            return a.type === 'markdown';
        });
...
function _renderSidebar() {
    ...
    const mdArticles = filtered.filter(function (a) {
        return a.type === 'markdown';
    });
    const htmlArticles = filtered.filter(function (a) {
        return a.type === 'html';
    });
    ...
    html +=
        '<span class="blog-nav-section-title">📘 Markdown <span class="blog-nav-count">' +
        mdArticles.length +
        '</span></span>';
    ...
    html += '<div class="blog-nav-section">';
    html +=
        '<span class="blog-nav-section-title">📄 HTML <span class="blog-nav-count">' +
        htmlArticles.length +
        '</span></span>';
...
function _groupArticles(list) {
    ...
    return {
        loose: loose,
        books: bookOrder.map(function (d) {
            return { name: d, items: books[d] };
        }),
    };
}
...
function _renderArticleGroup(list, type, queryActive) {
    ...
    g.books.forEach(function (book) {
        const open =
            queryActive ||
            book.items.some(function (a) {
                return _currentFile === a.name;
            });
        parts += '<details class="blog-nav-book"' + (open ? ' open' : '') + '>';
        parts +=
            '<summary class="blog-nav-book-title"><span class="blog-nav-book-icon">▸</span>' +
            Utils.escapeHtml(book.name) +
            ' <span class="blog-nav-count">' +
            book.items.length +
            '</span></summary>';
        parts += '<ul class="blog-nav-list">';
 ...
 */ async function _selectArticle(filename, type) {
    // 无论 HTML 还是 Markdown 分支，都先取消在途请求并递增请求 ID：
    // 否则 HTML 分支直接 return，会让上一个 Markdown 请求通过 ID 校验，
    // 其迟到响应会覆盖"已在新标签页打开"占位区并回滚侧栏高亮。
    ...
    if (type === 'html') {
        ...
        if ($blogContent) {
            $blogContent.innerHTML =
                ...
                '<div class="blog-content-placeholder-hint">' +
    ...
    try {
        const resp = await fetch(
            API.MARKDOWN_FILE + filename.split('/').map(encodeURIComponent).join('/'),
            { signal: signal }
        );
        ...
        $blogContent.innerHTML =
            '<div class="md-error">渲染失败: ' + Utils.escapeHtml(err.message) + '</div>';
        if ($blogToc) $blogToc.innerHTML = '';
    } finally {
...
function _onFilterClick(e) {
    ...
    if ($blogFilter) {
        $blogFilter.querySelectorAll('.blog-filter-btn').forEach(function (b) {
