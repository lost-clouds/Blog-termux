/**
 * @module tikz-renderer.test.cases-core
 * @description 基础图元与坐标回归（原 1–22 号用例）
...
/**
 * 运行 基础图元与坐标回归（原 1–22 号用例）。
 * @param {{render:Function, check:Function}} tools
 * @returns {Promise<void>}
 */
export async function runCoreCases({ render, check }) {
{
...
// 3. rect (0,0)-(2,3) -> SVG x:0..64, y:-96..0
{
    ...
    check(
        'rect (0,0)-(2,3) -> x0..64,y-96..0',
        f[0] === 0 && f[1] === -96 && f[2] === 64 && f[3] === 96,
        JSON.stringify(f)
    );
}
...
// 6. plot y=x^2 endpoints and vertex
{
    ...
    check(
        'plot y=x^2 endpoints (-64,-128)..(64,-128)',
        Math.abs(s[0] + 64) < 1e-6 &&
            Math.abs(s[1] + 128) < 1e-6 &&
            Math.abs(e[0] - 64) < 1e-6 &&
            Math.abs(e[1] + 128) < 1e-6,
        `s=${s} e=${e}`
    );
...
// 9. grid with step option: (0,0) grid (5,4) with step=1 → 6 vertical lines (x=0..5), 5 horizontal (y=0..4)
{
    ...
    check(
        'step=1 grid x lines 0,32,64,96,128,160',
        xvals.length === 6 && xvals[0] === 0 && xvals[5] === 160,
        `xvals=${JSON.stringify(xvals)}`
    );
}
...
{
    const svg = await render(
        '\\begin{document}\\begin{tikzpicture}[scale=1]\\draw[very thin,gray!30] (-4.5,-3.5) grid (4.5,3.5);\\draw[thick,->] (-4.5,0) -- (4.5,0);\\draw[thick,->] (0,-3.5) -- (0,3.5);\\fill (3,2) circle (2pt);\\end{tikzpicture}\\end{document}'
    );
    ...
    const cols = [...svg.matchAll(/<line x1="(-?\d+)" y1="(-?\d+)" x2="(-?\d+)"/g)]
        ...
        .sort((a, b) => a - b);
    check(
        'axis+grid: 9 columns at -128..128 step 32',
    ...
    check(
        'point (3,2) aligns at cx=96,cy=-64',
        circle && +circle[1] === 96 && +circle[2] === -64,
        svg.slice(0, 300)
    );
}
...
{
    const svg = await render(
        '\\begin{document}\\begin{tikzpicture}[scale=1]\\draw[thick,->] (-3,0) -- (3,0);\\draw[thick,->] (0,-3) -- (0,5);\\draw[blue,domain=-2:2] plot (\\x,{2*\\x+1});\\fill (0,1) circle (2pt);\\fill (1,3) circle (2pt);\\end{tikzpicture}\\end{document}'
    );
    const circles = [...svg.matchAll(/<circle cx="(-?[0-9.e-]+)" cy="(-?[0-9.e-]+)"/g)].map((m) => [
        Math.round(+m[1]),
        Math.round(+m[2]),
    ]);
    const ds = [...svg.matchAll(/<path d="([^"]+)"/g)]
        .map((m) => m[1])
        .filter((d) => d.split(' ').length > 20); // plot path has many coords
    const d = ds[0] || '';
    const pts = d
        .replace(/M/g, '')
        .split('L')
    ...
    const okPts =
        circles.some((c) => c[0] === 0 && c[1] === -32) &&
        circles.some((c) => c[0] === 32 && c[1] === -96);
    const okPlot = Math.abs(at0[1] - -32) < 1e-6;
    check(
        'composite: points + plot align',
        okPts && okPlot,
        `circles=${JSON.stringify(circles)} plot@0=${at0}`
    );
}
...
// 12. foreach variable substitution: points spread along x (NOT all at origin)
{
    ...
    const cs = [...svg.matchAll(/<circle cx="(-?[\d.]+)" cy="(-?[\d.]+)"/g)].map(
        (m) => Math.round(+m[1]) + ',' + Math.round(+m[2])
    );
    const xs = new Set(cs.map((c) => c.split(',')[0]));
    check(
        'foreach x spreads: 5 distinct x (0,32,64,96,128)',
...
// 13. mid-dot ellipsis {1,2,...,6} expands to 1..6 (previously produced NaN)
{
    ...
    check(
        'mid-dot ellipsis 1..6 -> 6 points 32..192',
...
{
    const svg = await render(
        '\\foreach \\x in {0,0.5,...,3} { \\fill[orange] (\\x,0) circle (1pt); };'
    );
    const cs = new Set([...svg.matchAll(/<circle cx="(-?[\d.]+)"/g)].map((m) => Math.round(+m[1])));
    check(
        'step ellipsis 0..3 step .5 -> 7 points 0..96',
...
{
    const svg = await render(
        '\\coordinate (o) at (0,0);\\coordinate (p) at (3,2);\\foreach \\pt in {o,p} { \\fill (\\pt) circle (2.5pt); };'
    );
    const cs = new Set(
        [...svg.matchAll(/<circle cx="(-?[\d.]+)" cy="(-?[\d.]+)"/g)].map(
            (m) => Math.round(+m[1]) + ',' + Math.round(+m[2])
        )
    );
    check(
        'string foreach {o,p} -> circles at (0,0) and (96,-64)',
...
{
    const svg = await render(
        '\\foreach \\x in {1,2,3} { \\pgfmathsetmacro{\\h}{\\x*0.6} \\draw[fill=teal!30] (\\x-0.4,0) rectangle (\\x+0.4,\\h); };'
    );
    const rs = [
        ...svg.matchAll(
...
{
    const svg = await render(
        '\\coordinate (A) at (0,0);\\coordinate (B) at (3,2);\\draw[->, red, thick] (A) -- (B);'
    );
    const paths = [...svg.matchAll(/<path d="([^"]+)"/g)].map((m) => m[1]);
    check(
        'coordinate path uses registered coords (M0 0L96 -64)',
...
// 19. inline node label on draw path (node below at end)
{
    ...
    check(
        'inline "node[below] {x}" at end (96, +8)',
        m && Math.round(+m[1]) === 96 && Math.round(+m[2]) === 8,
        'text at ' + (m && m[1] + ',' + m[2])
    );
}
...
// 20. inline node with at (x,y) override
{
    ...
    check(
        'inline "node[...] at (1,1)" label at (32,-40)',
        m && Math.round(+m[1]) === 32 && Math.round(+m[2]) === -40,
        'text at ' + (m && m[1] + ',' + m[2])
    );
}
...
// 21. mid-path node "a -- node{lab} b" sits at segment midpoint (48, 0)
{
    ...
    check(
        'mid-path "node{mid}" at (48, 0)',
        m && Math.round(+m[1]) === 48 && Math.round(+m[2]) === 0,
        'text at ' + (m && m[1] + ',' + m[2])
    );
}
...
// 22. combined anchor "above right" on circle label
{
    ...
    check(
        'circle label "above right" at (104,-72)',
        m && Math.round(+m[1]) === 104 && Math.round(+m[2]) === -72,
        'text at ' + (m && m[1] + ',' + m[2])
    );
}
}
