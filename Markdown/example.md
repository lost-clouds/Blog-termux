...
阅读器检测到 <code>```tikz</code> 代码块时，在**本地**将基础 TikZ 指令解析为 SVG 矢量图（纯客户端、零外部依赖，不发起任何网络请求）。

### 15.1 节点与箭头（流程图）

```tikz
\node (start) at (0,0) {开始};
\node (process) at (4,0) {处理};
\node (end) at (8,0) {结束};
\draw[->] (start) -- (process);
\draw[->] (process) -- (end);
```
...
```tikz
\node (q) at (0,0) {是否满足?};
\node[circle,draw] (y) at (4,2) {是};
\node[circle,draw] (n) at (4,-2) {否};
\draw[->] (q) -- node{满足} (y);
\draw[->] (q) -- node{不满足} (n);
```
...
\draw[red,dashed] (3,0) rectangle (6,3);
\draw[->,green] (0,0) -- (6,0);
```

### 15.4 网格

网格的两个坐标是**对角端点**（左下角与右上角），网格线按整数坐标（或 `step`）绘制，
可与坐标轴完全对齐：

```tikz
\draw (-4.5,-3.5) grid (4.5,3.5);
\draw[thick,->] (-4.5,0) -- (4.5,0);
\draw[thick,->] (0,-3.5) -- (0,3.5);
\fill (3,2) circle (2pt);
```

也可以指定网格步长，或仅填色不使用网格线：

```tikz
\draw[step=1] (0,0) grid (5,4);
\fill[blue!20!white] (6,0) grid (9,3);
```

### 15.5 语法错误 — 降级显示

不受支持的指令会被整体降级，保留源码并提示错误：
...
> 支持三类列表——显式列举 `{0,2,4}`、中置省略号步长 `{1,3,...,9}` / `{0,0.5,...,3}`、
> 以及遍历 `\coordinate` 命名坐标的字符串列表 `{o,p}`。
### 15.9 数学节点
