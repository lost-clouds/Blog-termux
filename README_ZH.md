...
| 🧠 CPU | 使用率% · 核心数 · 型号 · 集群负载 (Cortex-A73/Cortex-A53) | 蓝色 |
| 💾 内存 | used / total (GB/MB) + SWAP | 蓝色 |
...
> CPU 卡片新增每集群使用率子行（如 Cortex-A73 4核 2.4GHz 95%），集群名从 `lscpu` Model name 或 `/proc/cpuinfo` CPU part 识别。内存卡片新增 SWAP 行。
...
| 零外部依赖 | 所有库本地化在 `lib/` |
| 无 root | `corn.sh` 使用 `lscpu`/cpufreq sysfs/`/proc/stat`/`top`/`free`/`uptime`/`getprop`/`ps`（零 root 权限） |
